...
export default function Home() {
  ...
  const [featureAuthOpen, setFeatureAuthOpen] = useState(false);
  const [featureAuthReturn, setFeatureAuthReturn] = useState('/race');

  const openFeatureAuthGate = useCallback((returnHref: string) => {
    setFeatureAuthReturn(returnHref);
    setFeatureAuthOpen(true);
  }, []);
    ...
    <div className="fuel-page">
      ...
      <main className="fuel-main" style={S.main}>
          ...
          {FEATURE_CARDS.map((card) => {
            ...
            const borderAccent =
              card.accent === 'var(--color-accent)'
                ...
                : `${card.accent}66`;
            const accountOnly = pathnameRequiresAuthentication(card.href);
            const cardLocked = guestLocked && accountOnly;
            ...
            if (cardLocked) {
              return (
                <button
                  key={card.href}
                  type="button"
                  className="fuel-card fuel-card-interactive block w-full cursor-pointer text-left no-underline"
                  style={cardShellStyle}
                  aria-label={`${card.title} — ${NAV_ACCOUNT_REQUIRED_HINT}`}
                  title={NAV_ACCOUNT_REQUIRED_HINT}
                  onClick={() => openFeatureAuthGate(card.href)}
                >
                  {inner}
                </button>
              );
        ...
        {guestLocked ? (
          ...
          >
            Les pastilles « Compte » ouvrent la connexion : Plan et Produits restent utilisables sans compte.
          </p>
        ) : null}
      ...
      <RacesAuthGateModal
        open={featureAuthOpen}
        onClose={() => setFeatureAuthOpen(false)}
        returnTo={featureAuthReturn}
      />
    </div>
  );
}
