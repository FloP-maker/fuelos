                                                                                                                                                                          ...
                                                                                                                                                                          <div style={{ fontSize: 22, fontWeight: 800, color: i === 0 ? "#60a5fa" : i === 1 ? "#34d399" : "#fb923c" }}>
                                                                                                                                                                                          {phase.choPerHour}<span style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-muted)" }}>g/h</span>
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      <div style={{ fontSize: 11, color: "var(--color-text-muted)", marginTop: 2 }}>{phase.description}</div>
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    {/* 📈 CHO progression chart */}
                                                                                                                                                                                                                                                            <div style={{marginTop:16,marginBottom:4}}>
                                                                                                                                                                                                                                                              <div style={{fontSize:11,fontWeight:700,color:"var(--color-text-muted)",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.5px"}}>📈 Courbe CHO/h</div>
                                                                                                                                                                                                                                                              {(()=>{const p=plan.choStrategy!.phases;const total=p[p.length-1].endTimeMin;const mx=Math.max(...p.map(x=>x.choPerHour))*1.2;const W=360,H=70,PL=28,PR=8,PT=6,PB=16;const gW=W-PL-PR,gH=H-PT-PB;const tx=(m:number)=>PL+(m/total)*gW;const ty=(c:number)=>PT+gH-(c/mx)*gH;const C=["#60a5fa","#34d399","#fb923c"];let d="";p.forEach((ph,i)=>{const x1=tx(ph.startTimeMin),x2=tx(ph.endTimeMin),y=ty(ph.choPerHour);d+=i===0?`M${x1},${y} L${x2},${y}`:` L${x1},${y} L${x2},${y}`;});const aD=`${d} L${tx(p[p.length-1].endTimeMin)},${PT+gH} L${tx(p[0].startTimeMin)},${PT+gH} Z`;return(<svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",display:"block",overflow:"visible"}}><defs><linearGradient id="choG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--color-accent)" stopOpacity="0.2"/><stop offset="100%" stopColor="var(--color-accent)" stopOpacity="0"/></linearGradient></defs>{p.map((ph,i)=>(<rect key={i} x={tx(ph.startTimeMin)} y={PT} width={tx(ph.endTimeMin)-tx(ph.startTimeMin)} height={gH} fill={C[i%3]} fillOpacity={0.05}/>))}{[0.5,1].map((t,i)=>(<line key={i} x1={PL} x2={W-PR} y1={ty(mx*t)} y2={ty(mx*t)} stroke="var(--color-border)" strokeWidth="0.5" strokeDasharray="3 3"/>))}<line x1={PL} x2={PL} y1={PT} y2={PT+gH} stroke="var(--color-border)" strokeWidth="1"/><line x1={PL} x2={W-PR} y1={PT+gH} y2={PT+gH} stroke="var(--color-border)" strokeWidth="1"/>{[0.5,1].map((t,i)=>(<text key={i} x={PL-3} y={ty(mx*t)+3} textAnchor="end" fontSize="7" fill="var(--color-text-muted)">{Math.round(mx*t)}</text>))}<path d={aD} fill="url(#choG)"/><path d={d} fill="none" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>{p.map((ph,i)=>{const cx=tx((ph.startTimeMin+ph.endTimeMin)/2),cy=ty(ph.choPerHour);return(<g key={i}><circle cx={tx(ph.startTimeMin)} cy={cy} r={3} fill={C[i%3]} stroke="white" strokeWidth="1"/><text x={cx} y={cy-6} textAnchor="middle" fontSize="8" fontWeight="700" fill={C[i%3]}>{ph.choPerHour}g/h</text></g>)})}{p.map((ph,i)=>(<text key={i} x={tx(ph.startTimeMin)} y={H-2} textAnchor="middle" fontSize="7" fill="var(--color-text-muted)">{ph.startTimeMin}min</text>))}<text x={tx(p[p.length-1].endTimeMin)} y={H-2} textAnchor="end" fontSize="7" fill="var(--color-text-muted)">{p[p.length-1].endTimeMin}min</text></svg>);})()}
                                                                                                                                                                                                                                                            </div>
